...
docker-machine ssh dev "docker build -t ikovalyov/influx:latest /home/docker/docker/influx/"
...
docker-machine ssh dev "docker run --name influxdb --volumes-from websites -d -p 8083:8083 -p 8086:8086 ikovalyov/influx"
docker-machine ssh dev "docker run -dt -p 9000:9000 -p 9001:9001 -p 10022:22 --name=php-xdebug --volumes-from websites --link mongo --link mysql --link memcache --link influxdb --add-host admin.dnsperf.int:192.168.99.100 --add-host node1.dnsperf.int:192.168.99.100  ikovalyov/php-xdebug"
